...
/// An `Item` type that defines the Brigandine, an armor.
///
class BrigandineItem: PassiveItem, TradableItem, AlterationItem, LevelItem, TextureUser {
    
    static var textureNames: Set<String> {
        return [IconSet.Item.copperHauberk.imageName]
    }
    
    let name: String = "Brigandine"
    let icon: Icon = IconSet.Item.copperHauberk
    let category: ItemCategory = .armor
    let isUnique: Bool = false
    let isDiscardable: Bool = true
    let isEquippable: Bool = true
    var price: Int { return calculatePrice(basePrice: 50) }
    ...
    ///
    private init(other: BrigandineItem) {
    ...
    func copy() -> Item {
        return BrigandineItem(other: self)
    }
