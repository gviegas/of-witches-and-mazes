...
/// An `Item` type that defines the Leather Vest, an armor.
///
class LeatherVestItem: PassiveItem, TradableItem, AlterationItem, LevelItem, TextureUser {
    
    static var textureNames: Set<String> {
        return [IconSet.Item.brownTunic.imageName]
    }
    
    let name: String = "Leather Vest"
    let icon: Icon = IconSet.Item.brownTunic
    let category: ItemCategory = .armor
    let isUnique: Bool = false
    let isDiscardable: Bool = true
    let isEquippable: Bool = true
    var price: Int { return calculatePrice(basePrice: 42) }
    ...
    ///
    private init(other: LeatherVestItem) {
    ...
    func copy() -> Item {
        return LeatherVestItem(other: self)
    }
