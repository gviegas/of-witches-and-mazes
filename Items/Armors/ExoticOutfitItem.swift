...
/// An `Item` type that defines the Exotic Outfit, an armor.
///
class ExoticOutfitItem: PassiveItem, TradableItem, AlterationItem, LevelItem, TextureUser {
    
    static var textureNames: Set<String> {
        return [IconSet.Item.steelCuirass.imageName]
    }
    
    let name: String = "Exotic Outfit"
    let icon: Icon = IconSet.Item.steelCuirass
    ...
    ///
    private init(other: ExoticOutfitItem) {
    ...
    required init(level: Int) {
        ...
        let possibleScales: [AlterableStat: (scale: Double, ratio: Double)] = [
            .ability(.strength): (0.35, 0.15),
            .ability(.agility): (0.35, 0.15),
            .ability(.intellect): (0.35, 0.15),
            .ability(.faith): (0.35, 0.15)]
    ...
    func copy() -> Item {
        return ExoticOutfitItem(other: self)
    }
