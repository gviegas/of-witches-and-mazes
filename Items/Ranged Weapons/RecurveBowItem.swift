...
/// An `Item` that defines the Recurve Bow, a ranged weapon.
///
class RecurveBowItem: UsableItem, PassiveItem, TradableItem, ResourceItem, DamageItem, AlterationItem,
LevelItem, TextureUser {
    
    static var textureNames: Set<String> {
        return [IconSet.Item.recurveBow.imageName]
    }

    let name: String = "Recurve Bow"
    let icon: Icon = IconSet.Item.recurveBow
    ...
    ///
    private init(other: RecurveBowItem) {
    ...
    required init(level: Int) {
        ...
        let damage = Damage(scale: 1.0, ratio: 0.3, level: level,
                            modifiers: [.strength: 0.1, .agility: 0.35],
    ...
    func copy() -> Item {
        return RecurveBowItem(other: self)
    }
    
    func didUse(onEntity entity: Entity) {
        guard let stateComponent = entity.component(ofType: StateComponent.self) else {
            fatalError("RecurveBowItem can only be used by an entity that has a StateComponent")
        }
        guard let missileComponent = entity.component(ofType: MissileComponent.self) else {
            fatalError("RecurveBowItem can only be used by an entity that has a MissileComponent")
        }
