...
/// An `Item` that defines the Ashen Bow, a ranged weapon.
///
class AshenBowItem: UsableItem, PassiveItem, TradableItem, ResourceItem, DamageItem, AlterationItem, LevelItem,
TextureUser {
    
    static var textureNames: Set<String> {
        return [IconSet.Item.whiteRecurveBow.imageName]
    }
    
    let name: String = "Ashen Bow"
    let icon: Icon = IconSet.Item.whiteRecurveBow
    ...
    ///
    private init(other: AshenBowItem) {
    ...
    required init(level: Int) {
        ...
        let damage = Damage(scale: 1.1, ratio: 0.3, level: level,
                            modifiers: [.strength: 0.15, .agility: 0.4],
    ...
    func copy() -> Item {
        return AshenBowItem(other: self)
    }
    
    func didUse(onEntity entity: Entity) {
        guard let stateComponent = entity.component(ofType: StateComponent.self) else {
            fatalError("AshenBowItem can only be used by an entity that has a StateComponent")
        }
        guard let missileComponent = entity.component(ofType: MissileComponent.self) else {
            fatalError("AshenBowItem can only be used by an entity that has a MissileComponent")
        }
