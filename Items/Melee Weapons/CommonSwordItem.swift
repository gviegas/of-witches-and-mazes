...
/// An `Item` that defines the Common Sword, a melee weapon.
///
class CommonSwordItem: UsableItem, PassiveItem, TradableItem, DamageItem, AlterationItem, LevelItem,
TextureUser {

    static var textureNames: Set<String> {
        return [IconSet.Item.commonSword.imageName]
    }
    
    let name: String = "Common Sword"
    let icon: Icon = IconSet.Item.commonSword
    ...
    ///
    private init(other: CommonSwordItem) {
    ...
    func copy() -> Item {
        return CommonSwordItem(other: self)
    }
    
    func didUse(onEntity entity: Entity) {
        guard let stateComponent = entity.component(ofType: StateComponent.self) else {
            fatalError("CommonSwordItem can only be used by an entity that has a StateComponent")
        }
        guard let attackComponent = entity.component(ofType: AttackComponent.self) else {
            fatalError("CommonSwordItem can only be used by an entity that has an AttackComponent")
        }
