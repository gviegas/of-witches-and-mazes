...
/// An `Item` that defines the Rapier, a melee weapon.
///
class RapierItem: UsableItem, PassiveItem, TradableItem, DamageItem, AlterationItem, LevelItem, TextureUser {
    
    static var textureNames: Set<String> {
        return [IconSet.Item.rapier.imageName]
    }
    
    let name: String = "Rapier"
    let icon: Icon = IconSet.Item.rapier
    ...
    required init(level: Int) {
        ...
        let damage = Damage(scale: 1.0, ratio: 0.35, level: level,
                            modifiers: [.strength: 0.25, .agility: 0.3],
    ...
    func copy() -> Item {
        return RapierItem(other: self)
    }
    
    func didUse(onEntity entity: Entity) {
        guard let stateComponent = entity.component(ofType: StateComponent.self) else {
            fatalError("RapierItem can only be used by an entity that has a StateComponent")
        }
        guard let attackComponent = entity.component(ofType: AttackComponent.self) else {
            fatalError("RapierItem can only be used by an entity that has an AttackComponent")
        }
