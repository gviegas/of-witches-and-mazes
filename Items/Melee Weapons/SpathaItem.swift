...
/// An `Item` that defines the Spatha, a melee weapon.
///
class SpathaItem: UsableItem, PassiveItem, TradableItem, DamageItem, AlterationItem, LevelItem, TextureUser {
    
    static var textureNames: Set<String> {
        return [IconSet.Item.spatha.imageName]
    }
    
    let name: String = "Spatha"
    let icon: Icon = IconSet.Item.spatha
    ...
    required init(level: Int) {
        ...
        let damage = Damage(scale: 1.25, ratio: 0.3, level: level,
                            modifiers: [.strength: 0.15, .agility: 0.15],
    ...
    func copy() -> Item {
        return SpathaItem(other: self)
    }
    
    func didUse(onEntity entity: Entity) {
        guard let stateComponent = entity.component(ofType: StateComponent.self) else {
            fatalError("SpathaItem can only be used by an entity that has a StateComponent")
        }
        guard let attackComponent = entity.component(ofType: AttackComponent.self) else {
            fatalError("SpathaItem can only be used by an entity that has an AttackComponent")
        }
