...
/// An `EntityState` subclass representing the state of a `Companion` when following a master.
///
class CompanionFollowState: EntityState {
    
    private var movementComponent: MovementComponent {
        guard let component = entity.component(ofType: MovementComponent.self) else {
            fatalError("An entity assigned to CompanionFollowState must have a MovementComponent")
        }
        return component
    }
    ...
    private var directionComponent: DirectionComponent {
        guard let component = entity.component(ofType: DirectionComponent.self) else {
            fatalError("An entity assigned to CompanionFollowState must have a DirectionComponent")
        }
        return component
    }
    ...
    private var spriteComponent: SpriteComponent {
        guard let component = entity.component(ofType: SpriteComponent.self) else {
            fatalError("An entity assigned to CompanionFollowState must have a SpriteComponent")
        }
        return component
    }
    ...
    private var physicsComponent: PhysicsComponent {
        guard let component = entity.component(ofType: PhysicsComponent.self) else {
            fatalError("An entity assigned to CompanionFollowState must have a PhysicsComponent")
        }
        return component
    }
    ...
    private var companionComponent: CompanionComponent {
        guard let component = entity.component(ofType: CompanionComponent.self) else {
            fatalError("An entity assigned to CompanionFollowState must have a CompanionComponent")
        }
        return component
    }
    ...
    /// The minimum distance from the goal.
    ///
    var minDistance: CGFloat {
        return companionComponent.distance.lowerBound
    }
    ...
    override func didEnter(from previousState: GKState?) {
        // Leave immediately if has no companion
        guard companionComponent.companion != nil else {
    ...
    /// a new path to the goal will be computed or, if it is within range of the goal, stops.
    /// If the goal is still the same or not enough time has passed, it will continue following the
    /// current path. In this case, it will keep checking for three possible situations:
    ///
    /// 1. It is within range of the goal, in which case it will stop.
    /// 2. There are points to follow along the path, so stay on path.
    /// 3. The end of the path has been reached and it is not within range of the goal, because it did not stop
    /// in (1). In this case, it must be adjacent to the goal but its minimum distance is less than the
    /// current distance. Thus, it will instead moves towards the goal itself, and not along a path.
    ///
    ...
    override func update(deltaTime seconds: TimeInterval) {
        guard let newGoal = companionComponent.position, companionComponent.isFar != true else {
            stopFollowing()
            return
        }
        ...
        // Set the goal as the companion's current position
        goal = newGoal
        ...
        if (elapsedTime >= evaluationDelay) /*&& (goal != previousGoal)*/ {
            ...
            if withinRange(pointA: origin, pointB: goal!, offset: minDistance) {
                // Within range of the companion, stop
                stopFollowing()
            } else {
                // Try to find a new path
                let _path = entity.level?.findPathFrom(origin, to: goal!)
                if _path == nil {
                    // No path could be found, stop
                    stopFollowing()
                } else {
                    path = _path!
                    if path.isEmpty {
                        // The companion is visible but out of range, move towards the goal
                        path.append(goal!)
                    }
            ...
            let origin = physicsComponent.position
            if withinRange(pointA: origin, pointB: goal!, offset: minDistance) {
                // Close enough, no need to keep following the path - just stop
                stopFollowing()
    ...
    /// Stops following and leaves the chase state.
    ///
