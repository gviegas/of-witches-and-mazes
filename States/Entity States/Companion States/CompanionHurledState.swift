...
/// An `EntityState` subclass representing the state of a `Companion` when hurled.
///
class CompanionHurledState: EntityState, Contactable {
    
    private var nodeComponent: NodeComponent {
        guard let component = entity.component(ofType: NodeComponent.self) else {
            fatalError("An entity assigned to CompanionHurledState must have a NodeComponent")
        }
        return component
    }
    ...
    private var physicsComponent: PhysicsComponent {
        guard let component = entity.component(ofType: PhysicsComponent.self) else {
            fatalError("An entity assigned to CompanionHurledState must have a PhysicsComponent")
        }
        return component
    }
    ...
    private var shadowComponent: ShadowComponent {
        guard let component = entity.component(ofType: ShadowComponent.self) else {
            fatalError("An entity assigned to CompanionHurledState must have a ShadowComponent")
        }
        return component
    }
    ...
    private var interactionComponent: InteractionComponent {
        guard let component = entity.component(ofType: InteractionComponent.self) else {
            fatalError("An entity assigned to CompanionHurledState must have an InteractionComponent")
        }
        return component
    }
    ...
    private var liftableComponent: LiftableComponent {
        guard let component = entity.component(ofType: LiftableComponent.self) else {
            fatalError("An entity assigned to CompanionHurledState must have a LiftableComponent")
        }
        return component
    }
    ...
    func contactDidBegin(_ contact: Contact) {
        if let other = contact.otherBody.node?.entity as? Entity, let damage = liftableComponent.hurlDamage {
            Combat.carryOutHostileAction(using: .none, on: other, as: liftableComponent.liftSubject,
