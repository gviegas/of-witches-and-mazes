//
//  UILabeledConfirmationElement.swift
...
/// An `UIElement` type that displays two pieces of text and two options.
///
class UILabeledConfirmationElement: UIElement {
    
    /// The node that holds all the element contents.
    ///
    private let contents: SKNode
    
    /// The middle label background.
    ///
    private let middleLabelBackground: UIBackground
    ...
    init(contentOffset: CGFloat, topLabelSize: CGSize, middleLabelSize: CGSize, bottomLabelSize: CGSize,
        ...
        let middleRect = CGRect(x: middleX, y: bottomLabelSize.height + contentOffset,
                              width: middleLabelSize.width, height: middleLabelSize.height)
        middleLabel = UIText(rect: CGRect(origin: CGPoint.zero, size: middleRect.size),
                             style: .value, text: nil, alignment: .center)
        
        // Create the middle background
        middleLabelBackground = UIBackground.defaultBlackBackground(rect: middleRect)
        let middleLabelNode = SKNode()
        middleLabelNode.zPosition = 1
        middleLabelNode.addChild(middleLabel.node)
        middleLabelNode.position.x -= middleLabelBackground.node.size.width / 2.0
        middleLabelNode.position.y -= middleLabelBackground.node.size.height / 2.0
        middleLabelBackground.node.addChild(middleLabelNode)
        ...
        node.addChild(middleLabelBackground.node)
    ...
    /// Adds tracking data for the middle label.
    ///
    /// - Parameter data: The data to add.
    /// - Returns: `true` if the data was added, false otherwise.
    ///
    @discardableResult
    func addTrackinDataForMiddleLabel(data: Any) -> Bool {
        return addTrackingDataForNode(middleLabelBackground.node, data: data)
    }
