//
//  VolcanoTileSet.swift
//  Of Witches and Mazes
//
//  Created by Gustavo C. Viegas on 4/8/19.
//  Copyright © 2019 Gustavo C. Viegas. All rights reserved.
//

import SpriteKit

/// The `TileSet` representing the Volcano.
///
class VolcanoTileSet: TileSet, TextureUser {
    
    static var textureNames: Set<String> {
        let floor = ImageArray.createFrom(baseName: "Volcano_Floor_", first: 1, last: 10)
        let wall = ImageArray.createFrom(baseName: "Volcano_Wall_", first: 1, last: 4)
        let innerCorner = ImageArray.createFrom(baseName: "Volcano_Inner_Corner_", first: 1, last: 2)
        let outerCorner = ImageArray.createFrom(baseName: "Volcano_Outer_Corner_", first: 1, last: 2)
        return Set<String>(floor + wall + innerCorner + outerCorner)
    }
    
    /// The instance of the class.
    ///
    static let instance = VolcanoTileSet()
    ...
    func makeFloor(zRotation: CGFloat) -> Tile {
        let image = "Volcano_Floor_\(Int.random(in: 1...10))"
    ...
    func makeWall(zRotation: CGFloat) -> Tile {
        let image = "Volcano_Wall_\(Int.random(in: 1...4))"
    ...
    func makeInnerCorner(zRotation: CGFloat) -> Tile {
        let image = "Volcano_Inner_Corner_\(Int.random(in: 1...2))"
    ...
    func makeOuterCorner(zRotation: CGFloat) -> Tile {
        let image = "Volcano_Outer_Corner_\(Int.random(in: 1...2))"
