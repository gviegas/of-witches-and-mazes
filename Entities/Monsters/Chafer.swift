...
/// The Chafer entity, a monster.
///
class Chafer: Monster, TextureUser, AnimationUser {
    
    static var animationKeys: Set<String> {
        return ChaferAnimationSet.animationKeys
    }
    
    static var textureNames: Set<String> {
        return ChaferAnimationSet.textureNames.union([PortraitSet.chafer.imageName])
    }
    ...
    init(levelOfExperience: Int) {
        let data = ChaferData(levelOfExperience: levelOfExperience)
        ...
        let meleeDamage = Damage(scale: 0.45, ratio: 0.25, level: levelOfExperience,
                                 modifiers: [.strength: 0.2, .agility: 0.15],
                                 type: .physical, sfx: SoundFXSet.FX.weakHit)
        let attack = Attack(medium: .melee, damage: meleeDamage,
                            reach: 24.0, broadness: 24.0, delay: 0.35, duration: 0.1, conclusion: 0.4,
...
/// The `MonsterData` defining the data associated with the the `Chafer` entity.
///
fileprivate class ChaferData: MonsterData {
    ...
    init(levelOfExperience: Int) {
        ...
        progressionValues = ChaferProgressionValues.instance
        animationSet = ChaferAnimationSet()
        portrait = PortraitSet.chafer
        shadow = (CGSize(width: 18.0, height: 12.0), "Shadow")
        voice = (SoundFXSet.Voice.insect, .normal)
    }
}

/// The `EntityProgressionValues` subclass defining the progression values of the `Chafer` entity.
///
fileprivate class ChaferProgressionValues: EntityProgressionValues {
    
    /// The instance of the class.
    ///
    static let instance = ChaferProgressionValues()
    
    private init() {
        let abilityValues = [
            Ability.strength: ProgressionValue(initialValue: 2, rate: 0.15),
            Ability.agility: ProgressionValue(initialValue: 2, rate: 0.2),
