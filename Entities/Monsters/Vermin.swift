...
/// The Vermin entity, a monster.
///
class Vermin: Monster, TextureUser, AnimationUser {
    
    static var animationKeys: Set<String> {
        return VerminAnimationSet.animationKeys
    }
    
    static var textureNames: Set<String> {
        return VerminAnimationSet.textureNames.union([PortraitSet.vermin.imageName])
    }
    ...
    init(levelOfExperience: Int) {
        let data = VerminData(levelOfExperience: levelOfExperience)
        ...
        let meleeDamage = Damage(scale: 0.4, ratio: 0.25, level: levelOfExperience,
                                 modifiers: [.strength: 0.1],
                                 type: .physical, sfx: SoundFXSet.FX.weakHit)
        let attack = Attack(medium: .melee, damage: meleeDamage,
                            reach: 24.0, broadness: 24.0, delay: 0.4, duration: 0.1, conclusion: 0.6,
...
/// The `MonsterData` defining the data associated with the `Vermin` entity.
///
fileprivate class VerminData: MonsterData {
    ...
    init(levelOfExperience: Int) {
        ...
        progressionValues = VerminProgressionValues.instance
        animationSet = VerminAnimationSet()
        portrait = PortraitSet.vermin
        shadow = (CGSize(width: 18.0, height: 12.0), "Shadow")
        voice = nil
    }
}

/// The `EntityProgressionValues` subclass defining the progression values of the `Vermin` entity.
///
fileprivate class VerminProgressionValues: EntityProgressionValues {
    
    /// The instance of the class.
    ///
    static let instance = VerminProgressionValues()
    ...
    private init() {
        ...
        let healthPointsValue = ProgressionValue(initialValue: 6, rate: 3.15)
